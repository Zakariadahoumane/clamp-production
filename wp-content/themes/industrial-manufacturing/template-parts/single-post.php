...
<article>
	...
	<?php if( get_theme_mod( 'industrial_manufacturing_single_post_date',true) != '' || get_theme_mod( 'industrial_manufacturing_single_post_author',true) != '' || get_theme_mod( 'industrial_manufacturing_single_post_comment_no',true) != '' || get_theme_mod( 'industrial_manufacturing_single_post_time',true) != '') { ?>
	    <div class="post-info p-2 mb-2">
	      <?php if( get_theme_mod( 'industrial_manufacturing_single_post_date',true) != '') { ?>
	        <i class="<?php echo esc_attr(get_theme_mod('industrial_manufacturing_singlepost_date_icon',"fa fa-calendar pe-2" )); ?>"></i><a href="<?php echo esc_url( get_day_link( $archive_year, $archive_month, $archive_day)); ?>"><span class="entry-date pe-3"><?php echo esc_html( get_the_date() ); ?></span><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a>
	      <?php }?>
	      <?php if( get_theme_mod( 'industrial_manufacturing_single_post_author',true) != '') { ?>
	        <i class="<?php echo esc_attr(get_theme_mod('industrial_manufacturing_singlepost_author_icon',"fa fa-user pe-2" )); ?>"></i><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><span class="entry-author pe-3"> <?php the_author(); ?></span><span class="screen-reader-text"><?php the_author(); ?></span></a>
	      <?php }?>
	      <?php if( get_theme_mod( 'industrial_manufacturing_single_post_comment_no',true) != '') { ?>
	        <i class="<?php echo esc_attr(get_theme_mod('industrial_manufacturing_single_post_comment_icon',"fa fa-comments pe-2" )); ?>"></i><span class="entry-comments pe-3"> <?php comments_number( __('0 Comments','industrial-manufacturing'), __('0 Comments','industrial-manufacturing'), __('% Comments','industrial-manufacturing') ); ?></span> 
	      <?php }?>
	      <?php if( get_theme_mod( 'industrial_manufacturing_single_post_time',true) != '') { ?>
        	<span class="entry-comments me-2"><i class="<?php echo esc_attr(get_theme_mod('industrial_manufacturing_single_post_time_icon',"fa fa-clock pe-2" )); ?>"></i> <?php echo esc_html( get_the_time() ); ?></span>
        <?php }?>
	    </div>
	<?php }?>
	<?php if( get_theme_mod( 'industrial_manufacturing_single_post_image',true) != '' && get_theme_mod('industrial_manufacturing_post_featured_image','in-content') == 'in-content') { ?>
		...
		<?php } ?>
	<?php }?>
	<?php if( get_theme_mod( 'industrial_manufacturing_single_post_category',true) != '') { ?>
		<div class="single-post-category mt-3">
				<span class="category"><?php esc_html_e('Categories:','industrial-manufacturing'); ?></span>
			<?php the_category();?>
		</div> 
	<?php }?>
